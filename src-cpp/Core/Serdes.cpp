                          ...
                          std::string &errstr)
{
    ...
    serdes_schema_t *c_schema =
      serdes_schema_get(hnd->sd_, name, id, c_errstr, sizeof(c_errstr));
    if (!c_schema) {
        errstr = c_errstr;
        return NULL;
    }
...
static Schema *schema_add(Handle *handle, const char *name, int id,
                          ...
                          int definition_len, std::string &errstr)
{
    HandleImpl *hnd = dynamic_cast<HandleImpl *>(handle);
    char c_errstr[512];

    serdes_schema_t *c_schema =
      serdes_schema_add(hnd->sd_, name, id, type, definition, definition_len,
...
Schema *Schema::add(Handle *handle, int id, const std::string &type,
                    const std::string &definition, std::string &errstr)
{
    return schema_add(handle, NULL, id, type.c_str(), NULL, 0, errstr);
}
