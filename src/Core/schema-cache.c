 ...
 */
static int serdes_schema_store(serdes_schema_t *ss, const void *definition,
                               ...
                               int errstr_size)
{
    ...
    if (ss->ss_type)
        json_object_set_new(json, "schemaType", json_string(ss->ss_type));
 ...
 * Returns -1 on failure.
 */
static int serdes_schema_fetch(serdes_schema_t *ss, char *errstr,
                               int errstr_size)
{
    serdes_t *sd = ss->ss_sd;
    rest_response_t *rr;
    ...
    if (ss->ss_id != -1) {
        ...
        rr = rest_get(&sd->sd_conf.schema_registry_urls, "/schemas/ids/%d",
                      ss->ss_id);
    } else {
    ...
    if (!(json_schema = json_object_get(json, "schema")) ||
        ...
        snprintf(errstr, errstr_size,
                 "No \"schema\" string field in schema %d envelope", ss->ss_id);
        rest_response_destroy(rr);
        if (json_schema)
    ...
    serdes_schema_set_definition(ss, json_string_value(json_schema),
                                 json_string_length(json_schema));
    json_decref(json_schema);

    DBG(ss->ss_sd, "SCHEMA_FETCH",
        "Succesfully fetched schema %s id %d type %s: %s",
        ss->ss_name ? ss->ss_name : "(unknown-subject)", ss->ss_id, ss->ss_type,
        ss->ss_definition);
 ...
 */
static serdes_schema_t *serdes_schema_add0(serdes_t *sd, const char *name,
                                           ...
                                           int errstr_size)
{
    ...
    if (definition) {
        ...
        if (ss->ss_id == -1) {
            if (serdes_schema_store(ss, definition, definition_len, errstr,
                                    errstr_size) == -1) {
                serdes_schema_destroy0(ss);
                return NULL;
            }
...
static serdes_schema_t *serdes_schema_find_by_definition(serdes_t *sd,
                                   ...
                                   int errstr_size)
{
    ...
    pthread_mutex_lock(&sd->sd_lock);
    if (!(ss = serdes_schema_find_by_definition(sd, definition, definition_len,
                                                type, 0 /*no-lock*/)))
        ss = serdes_schema_add0(sd, name, id, type, definition, definition_len,
                                errstr, errstr_size);
    pthread_mutex_unlock(&sd->sd_lock);
                                   ...
                                   char *errstr, int errstr_size)
{
    ...
    ss = serdes_schema_add0(sd, name, id, NULL, NULL, 0, errstr, errstr_size);
    pthread_mutex_unlock(&sd->sd_lock);
