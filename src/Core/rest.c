...
char *url_encode(const char *orig)
{
     ...
     *          s^       ^t1      ^t2 */
    if ((t2 = strchr(s, '@')) && (t1 = strchr(s, ':')) && t1 < t2) {
        CURL *curl = curl_easy_init();
        int user_len = (int)(t1 - s);
        int pass_len = (int)(t2 - (t1 + 1));
        char *user = user_len > 0 ? curl_easy_escape(curl, s, user_len) : "";
        char *pass =
          pass_len > 0 ? curl_easy_escape(curl, t1 + 1, pass_len) : "";
        size_t size;

        s = t2 + 1;
        size =
          strlen(proto) + strlen(user) + 1 + strlen(pass) + 1 + strlen(s) + 1;
 ...
 */
char *rest_response_strerror(const rest_response_t *rr, char *errstr,
                             int errstr_size)
{
    if (rr->errstr)
        snprintf(errstr, errstr_size, "REST request failed (code %ld): %s",
                 rr->code, rr->errstr);
    else
        snprintf(errstr, errstr_size, "REST request failed (code %ld): %.*s",
...
static CURLcode rest_req_curl(CURL *curl, rest_response_t *rr)
{
    ...
    if (ccode != CURLE_OK) {
        rest_response_set_result(rr, -1, "HTTP request failed: %s",
                                 curl_easy_strerror(ccode));
    } else {
        if (curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &rr->code) !=
            CURLE_OK)
...
#define do_curl_setopt(curl, opt, val...)                                  \
    do {                                                                   \
        CURLcode _ccode = curl_easy_setopt(curl, opt, val);                \
        if (_ccode != CURLE_OK) {                                          \
            rest_response_set_result(rr, -1, "curl: setopt %s failed: %s", \
                                     #opt, curl_easy_strerror(_ccode));    \
            if (hdrs)                                                      \
    ...
    /* Set up cURL request */
    hdrs =
      curl_slist_append(hdrs, "Accept: application/vnd.schemaregistry.v1+json");
    hdrs = curl_slist_append(
      hdrs, "Content-Type: application/vnd.schemaregistry.v1+json");
...
rest_response_t *rest_post(url_list_t *ul, const void *payload, int size,
